...
enum ToolDefinitions {
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "crop": [
                        "type": "object",
                        "description": "Static source crop for video, image, Lottie, and nested timeline clips. Partial; omitted edges keep current values. Insets are 0–1 of the source (left/top/right/bottom). All zeros restore the full source. Clears crop keyframes. Not for split screen/PIP/grid — use apply_layout.",
                        "properties": [
                            "left": ["type": "number", "description": "Inset from the source left edge, 0–1."],
                            "top": ["type": "number", "description": "Inset from the source top edge, 0–1."],
                            "right": ["type": "number", "description": "Inset from the source right edge, 0–1."],
                            "bottom": ["type": "number", "description": "Inset from the source bottom edge, 0–1."],
                        ],
                    ],
                    "blendMode": [
                        "type": "string",
                    ...
                    "inAnimation": clipAnimationSchema(
                        "Entrance over the clip's first durationFrames. A slide travels in the named direction from off canvas to the clip's placement; zoomIn grows from half size and zoomOut shrinks from 1.5x, both fading in."
                    ),
                    "outAnimation": clipAnimationSchema(
                        "Exit over the clip's last durationFrames. A slide travels in the named direction until the clip is fully off canvas; zoomIn grows to 1.5x and zoomOut shrinks to half size, both fading out."
                    ),
                ],
                required: ["clipIds"]
            )
        ...
        AgentTool(
            ...
            inputSchema: objectSchema(
                properties: [
                    ...
                    "hideWhenClosed": ["type": "boolean", "description": "hands mode, default true. While the fingertips are touching, the four corners sit on top of each other and any shape drawn through them is noise — a visible spike. The path collapses to a point there instead, which draws nothing and lets the shape close and reopen with the hands. Set false to keep the raw quad."],
                    "phaseShapes": [
                        "type": "array",
                        "items": ["type": "array"],
                        "description": "hands mode only. One vertex ORDER per phase, cycled. Four indices, each 0–3, over the tracked corners sorted clockwise about their centre — so an order means the same thing on every frame however the hands turn. [0,1,2,3] is the plain quad the hands frame; [0,1,3,2] and [0,2,1,3] cross a pair of edges into a bowtie of two triangles. An index may REPEAT: [0,1,2,2] collapses the fourth corner onto the third, giving a triangle while keeping the vertex count at four, which is how a phase can change the apparent number of sides without breaking the track. Because a phase is an ordering rather than fixed coordinates, the shape keeps following the hands and the vertex count cannot change between phases. Phases are split at the frames the hands came together, returned as phaseBoundaries.",
                    ],
                ],
                required: ["clipId"]
            )
    ...
    private static func clipAnimationSchema(_ description: String) -> [String: Any] {
        [
            "type": "object",
            "description": description,
            "properties": [
                "preset": ["type": "string", "enum": ["none"] + ClipAnimation.Preset.allCases.map(\.rawValue)],
                "durationFrames": ["type": "integer", "minimum": 1, "description": "Required unless preset is 'none'."],
            ],
            "required": ["preset"],
        ]
    }
